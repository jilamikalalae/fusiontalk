    ...
    if (!response.ok) {
      ...
      if (data.error && data.error.code === 190) {
        return NewResponse(
          401,
          null,
          'Your Facebook access token has expired. Please reconnect your Meta account.'
        );
      }
      ...
      if (
        data.error &&
        data.error.code === 100 &&
        data.error.error_subcode === 2018001
      ) {
        return NewResponse(
          404,
          null,
          'User not found on Facebook. They may have blocked your page or deleted their account.'
        );
      }
